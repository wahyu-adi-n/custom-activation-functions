...
def pos_hill_v1(x, turn_point=0.75):
    # could possibly make the slopes another input variable and also potentially a trainable parameter
    # need to change both the up_slope and down_slope at the same time to maintain the continuity of the line
    t_zero = x.new([0.0])
    t_one = x.new([1.0])
...
class Pos_Hill_V1(nn.Module):
    def __init__(self, turn_point=0.75):
        super().__init__()
        self.turn_point = turn_point

    def forward(self, input):
        return pos_hill_v1(input, self.turn_point)
...
act_func_dict = {
                    ...
                    "small_neg" : small_neg,
                    "pos_hill_v1" : pos_hill_v1, 
                    "pos_hill_v2" : pos_hill_v2, 
                    "pos_hill_v3" : pos_hill_v3, 
                    "double_hill": double_hill, 
                    "val_hill" : val_hill
                }
...
if __name__ == '__main__':
    ...
    if make_plots == "Y":
        ...
        for name, func in act_func_dict.items():
            ...
            plt.title(name)
            plt.savefig(f"assets/afs/{name}_plot.png", format='png')
