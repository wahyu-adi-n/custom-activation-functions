...
with open("densenet_121_results.csv", mode="w") as csv_file:
    ...
    for text, func in afs_dict.items():
        ...
        for i in range(1, 2):
            ...
            plt.plot(epochs, train_accuracy_list, "bo", label="Train Accuracy")
            # plt.plot(epochs, val_acc, "b", label="Validation acc")
            plt.title(f"{text}_DenseNet_121_Train_Acc_{i}")
            plt.legend()
            plt.savefig(f"{text}_DenseNet_121_Train_Acc_{i}.png")
            plt.figure()
            plt.clf()

            plt.plot(epochs, test_accuracy_list, "b", label="Val Accuracy")
            # plt.plot(epochs, val_acc, "b", label="Validation acc")
            plt.title(f"assets/{text}_DenseNet_121_Val_Acc_{i}")
            plt.legend()
...
plt.plot(epochs, test_loss_list, "b", label="Val Loss")
